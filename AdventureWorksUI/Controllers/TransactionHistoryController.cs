...
namespace AdventureWorks.UI.Controllers
{
    public class TransactionHistoryController : Controller
    {
        ...
        private readonly string _baseUrl = "http://localhost:5217/api/TransactionHistory";

        public TransactionHistoryController(IHttpClientFactory httpClientFactory)
        {
            _httpClient = httpClientFactory.CreateClient();
        }

        // ✅ INDEX
        public async Task<IActionResult> Index(int pageNumber = 1, int pageSize = 20, string? sortBy = "TransactionDate", bool isDescending = true, int? productId = null)
        {
            ...
            var data = JsonConvert.DeserializeObject<List<TransactionHistoryViewModel>>(content);
            return View(data);
        }
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            var model = JsonConvert.DeserializeObject<TransactionHistoryViewModel>(json);
            return View(model);
        }
        ...
        // ✅ CREATE (POST)
        [HttpPost]
        public async Task<IActionResult> Create(TransactionHistoryViewModel model)
        {
        ...
        // ✅ EDIT (POST)
        [HttpPost]
        public async Task<IActionResult> Edit(int id, TransactionHistoryViewModel model)
        {
            if (!ModelState.IsValid) return View(model);
